        ...
        <table class="table-data">
            <tbody>
                ...
                <?php
                    ...
                    $currency_data_defalut = \App\Models\Currency::where('code', $currency_code)->first()->exchange_rate;
                ...
                ?>
                {{-- @php dd($product_sale_data, $lims_product_data); @endphp --}}
                <tr>
                    <th colspan="2" style="text-align:left">{{'Conversion Rate : '}}</th>
                    <th style="text-align:right">{{number_format((float)($currency_data_defalut), $general_setting->decimal, '.', '')}}</th>
                </tr>
            ...
            <!-- <tfoot> -->
                ...
                <tr style="background-color:#ddd;">
                    ...
                    <td style="padding: 5px;width:30%">{{trans('file.Due')}}: {{number_format((float)$payment_data->change, $general_setting->decimal, '.', '')}}</td>
                </tr>
                @endforeach

                @if($general_setting->pos_currency_check == 'yes')
                {{-- New code for currency start here currency_list--}}
                <tr><td class="centered" colspan="3">{{'Others Currency'}}</td></tr>
                {{-- @php dd($lims_product_data['price'], @$currency_data['exchange_rate']); @endphp --}}

                @foreach($currency_list as $currency_data)
                    
                    @if($currency_data['code'] != $currency_code)

                        <tr>
                            <th colspan="2" style="text-align:left">{{'Conversion Rate : '}}</th>
                            <th style="text-align:right">{{number_format((float)($currency_data['exchange_rate']), $general_setting->decimal, '.', '')}}</th>
                        </tr>

                        <tr>
                            <td colspan="2">

                                {{$product_sale_data->qty}} x {{number_format((float)(($lims_product_data['price']*$currency_data['exchange_rate'])), $general_setting->decimal, '.', '')}}
                            ...
                            <td style="text-align:right;vertical-align:bottom">{{number_format((float)(($product_sale_data->total*$currency_data['exchange_rate']) ), $general_setting->decimal, '.', '')}}</td>

                        </tr>

                        <tr>
                            <th colspan="2" style="text-align:left">{{trans('file.Total')}}</th>
                            <th style="text-align:right">{{number_format((float)($product_sale_data->total*$currency_data['exchange_rate']), $general_setting->decimal, '.', '')}}</th>
                        </tr>
                        ...
                        <tr>
                            @php $numberInWordsOther = $numberTransformer->toWords($product_sale_data->total*$currency_data['exchange_rate']); @endphp
                            @if($general_setting->currency_position == 'prefix')
                            <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{$currency_data['code']}}</span> <span>{{str_replace("-"," ",$numberInWordsOther)}}</span></th>
                            @else
                            <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{str_replace("-"," ",$numberInWordsOther)}}</span> <span>{{$currency_data['code']}}</span></th>
                            @endif
                        </tr>
                ...
                {{-- New code for currency end here --}}
                @endif
