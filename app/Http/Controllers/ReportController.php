            ...
            foreach ($sales as $key => $sale)
            {
                ...
                foreach ($product_sale_data as $index => $product_sale) {
                    ...
                    if($index)
                        ...
                        $nestedData['product'] = $product_sale->product_name.' ('.number_format($product_sale->qty, cache()->get('general_setting')->decimal).' '.$unitCode.')';
                    /*dd($sale->id);*/
                    $pay_method = payment::whereDate('created_at', '>=' , $request->input('start_date'))->whereDate('created_at', '<=' , $request->input('end_date'))->where('sale_id', $sale->id)->where('user_id', $user_id)->first();
                    /*dd($pay_method);*/
                    if($pay_method != null){
                        $nestedData['pay_method'] = $pay_method->paying_method;
                        // $nestedData['pay_method'] = 5;
                    } else {
                        $nestedData['pay_method'] = '-';
                    }
        ...
        if(!empty($purchases))
        {
            foreach ($purchases as $key => $purchase)
            {
                ...
                foreach ($product_purchase_data as $index => $product_purchase) {
                    ...
                    $pay_method = payment::whereDate('created_at', '>=' , $request->input('start_date'))->whereDate('created_at', '<=' , $request->input('end_date'))->where('purchase_id', $purchase->id)->where('user_id', $user_id)->first();
                    /*dd($pay_method);*/
                    if($pay_method != null){
                        $nestedData['pay_method'] = $pay_method->paying_method;
                    } else {
