...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        /* NEw code Start here */
        if(Auth::user()->id == 1 && $request['role_id'] == 1) {
            if($request->has('license')){
                $permission = Permission::firstOrCreate(['name' => 'license']);
                if(!$role->hasPermissionTo('license')) {
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('license');
        }
        if($request->has('pos_void')){
            $permission = Permission::firstOrCreate(['name' => 'pos_void']);
            if(!$role->hasPermissionTo('pos_void')) {
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('pos_void');
        /* NEw code End here */
